...
const CartPage = () => {
  ...
  const handleDelete = (record) => {
    ...
    const columns = [
      ...
      { title: 'Actions', dataIndex: "_id", render: (id, record) => <DeleteOutlined onClick={() => handleDelete(record)} /> }
  ];
  useEffect(()=>{
    let temp =0;
  ...
  return (
    ...
    <div className='d-flex justify-content-end'>
      <Button type='primary' htmlType='submit' shape="round" className="custom-button">Generate Bill</Button>
    </div>
    </Form>
    </Modal>
    </DefaultLayout>
